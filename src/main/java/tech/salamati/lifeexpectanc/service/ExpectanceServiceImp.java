...
public class ExpectanceServiceImp implements ExpectanceService {
    ...
    @Autowired
     private ExpectanceRepo _expectanceRepo;
    ...
    public AnonymousResponseDto anonymousResponse(AnonymousRequestDto dto) throws RequestNotMacheException {
        ...
        {
            result=_expectanceRepo.findByAgeAndSex(age+10,dto.getSex())
                    .orElseThrow(() -> new RequestNotMacheException("مشخصات وارده صحیح نیست"));
        }
        else {
            result = _expectanceRepo.findByAgeAndSex(age, dto.getSex())
                    .orElseThrow(() -> new RequestNotMacheException("مشخصات وارده صحیح نیست"));
        }
    ...
    public String getOverweight(AnonymousRequestDto dto) {
        ...
        else if (age>35 && age<44){normalBMI=24;}
        else if  (age>45 && age<54){normalBMI=26;}
        else if  (age>55 && age<64){normalBMI=27;}
        else if  (age>=65){normalBMI=28;}

        double normalWeight=normalBMI*(ghad*ghad);
        var overWeight= dto.getWeight()-normalWeight;

        if (overWeight>3 ){return  overWeight + " اضافه وزن ";}
        if (overWeight<4 ){return  overWeight + " کمبود وزن ";}
        return "وزن نرمال";
    }
